...
/**
 * A {@link WizardPage} prompting the user to enter upper, lower and if necessary, soft upper and
 * soft lower thresholds for a selected {@link ServiceLevelObjective}. All inputs are checked for
 * correctness.
 * 
...
public class SLODialogSpecifiySLOThresholdsPage extends WizardPage implements SelectionListener {
    ...
    /**
     * Time interval for qualitative SLO visualization, i.e. for calculating fulfillment ratio.
     */
    ...
    /**
     * The selected {@link ServiceLevelObjective} from the {@link SLODialogChooseSLOPage}
     */
    ...
    protected SLODialogSpecifiySLOThresholdsPage(String pageName) {
        ...
        inputStatus = new Status(IStatus.INFO, "not_used", 0,
                "Please specify the lower and upper thresholds of the Service Level Objective.", null);
    ...
    public void createControl(final Composite parent) {
        ...
        final ControlDecoration decorLower = new ControlDecoration(textLowerThreshold, SWT.CENTER);

        textLowerThreshold.addListener(SWT.Modify, new InputListener(textLowerThreshold, decorLower,
                "hardLowerThresholdSet"));
        ...
        final ControlDecoration decorUpper = new ControlDecoration(textUpperThreshold, SWT.CENTER);

        textUpperThreshold.addListener(SWT.Modify, new InputListener(textUpperThreshold, decorUpper,
                "hardUpperThresholdSet"));
        ...
        final ControlDecoration decorSoftUpper = new ControlDecoration(textSoftUpperThreshold, SWT.CENTER);

        textSoftUpperThreshold.addListener(SWT.Modify, new InputListener(textSoftUpperThreshold, decorSoftUpper,
        ...
        final ControlDecoration decorSoftLower = new ControlDecoration(textSoftLowerThreshold, SWT.CENTER);

        textSoftLowerThreshold.addListener(SWT.Modify, new InputListener(textSoftLowerThreshold, decorSoftLower,
    ...
    /**
     * Inner class handling user input. Checks input for correctness and sets the threshold values
     * accordingly.
     *
     */
    private class InputListener implements Listener {
         ...
         *            String specifying which threshold the text field is assigned to.
         */
        ...
        public void handleEvent(Event event) {
            try {
                ...
                if (this.thresholdReference.equals("hardLowerThresholdSet")) {
                    ...
                    hardLowerThreshold = Measure.valueOf(value, hardLowerThreshold.getUnit());
                    selectedSLO.getLowerThreshold().setThresholdLimit(hardLowerThreshold);

                    checkInputForCorrectness();
                }
                if (this.thresholdReference.equals("hardUpperThresholdSet")) {
                    hardUpperThresholdSet = true;
                    hardUpperThreshold = Measure.valueOf(value, hardUpperThreshold.getUnit());
                    selectedSLO.getUpperThreshold().setThresholdLimit(hardUpperThreshold);
                    checkInputForCorrectness();
                }
                if (this.thresholdReference.equals("softUpperThresholdSet")) {
                    softUpperThresholdSet = true;
                    softUpperThreshold = Measure.valueOf(value, softUpperThreshold.getUnit());
                    if (selectedSLO.getUpperThreshold() instanceof LinearFuzzyThreshold) {
                        ((LinearFuzzyThreshold) selectedSLO.getUpperThreshold()).setSoftLimit(softUpperThreshold);
                    }
                    checkInputForCorrectness();
                ...
                if (this.thresholdReference.equals("softLowerThresholdSet")) {
                    ...
                    softLowerThreshold = Measure.valueOf(value, softLowerThreshold.getUnit());
                    if (selectedSLO.getLowerThreshold() instanceof LinearFuzzyThreshold) {
                        ((LinearFuzzyThreshold) selectedSLO.getLowerThreshold()).setSoftLimit(softLowerThreshold);
                    }
                    checkInputForCorrectness();
                }

                // set page complete only if all thresholds are set
                if ((hardLowerThresholdSet && hardUpperThresholdSet && softLowerThreshold == null && softUpperThreshold == null)
                        || (hardLowerThresholdSet && hardUpperThresholdSet && softLowerThresholdSet && softUpperThreshold == null)
                        || (hardLowerThresholdSet && hardUpperThresholdSet && softLowerThreshold == null && softUpperThresholdSet)
                        || (hardLowerThresholdSet && hardUpperThresholdSet && softLowerThresholdSet && softUpperThresholdSet)) {
                    setPageComplete(true);
                    inputStatus = statusOK;
                    updatePageStatus();
                } else {
                    inputStatus = new Status(INFORMATION, "not used", "Please specify all thresholds.");
                    updatePageStatus();
                }
                ...
                inputStatus = new Status(IStatus.ERROR, "not_used", 0, "Non empty values of type double required.",
                        null);
    ...
    /**
     * Checks whether the input for thresholds is correct, i.e. if the hard upper threshold is the
     * largest value, the softUpperThreshold the second largest, softLowerThreshold the third
     * largest and hardLowerThreshold the smallest value. If not, a
     * {@link IncorrectThresholdInputException} is thrown.
     * 
    ...
    private void checkInputForCorrectness() throws IncorrectThresholdInputException {
        ...
        if (softLowerThreshold != null && hardLowerThreshold != null && hardUpperThreshold != null
    ...
    /**
     * Sets the selectedSLO and updates the UI accordingly, i.e. sets threshold input fields visible
     * or invisible and inserts default values.
     * 
     * @param selectedSLO
     */
    public void setSelectedSLO(ServiceLevelObjective selectedSLO) {
        ...
        if (lowerThreshold instanceof LinearFuzzyThreshold) {
            softLowerThreshold = (Measure<?, Quantity>) ((LinearFuzzyThreshold) lowerThreshold).getSoftLimit();
            labelSoftLowerThreshold
                    .setText("Soft lower threshold: [" + softLowerThreshold.getUnit().toString() + "]: ");
            labelSoftLowerThreshold.setVisible(true);
            Double softLowerThresholdDouble = softLowerThreshold.doubleValue(softLowerThreshold.getUnit());
            if (!textSoftLowerThreshold.getText().equalsIgnoreCase(softLowerThresholdDouble.toString())) {
                textSoftLowerThreshold.setText(softLowerThresholdDouble.toString());
                softLowerThresholdSet = true;
            }
        ...
        if (upperThreshold instanceof LinearFuzzyThreshold) {
            softUpperThreshold = (Measure<?, Quantity>) ((LinearFuzzyThreshold) upperThreshold).getSoftLimit();
            labelSoftUpperThreshold
                    .setText("Soft upper threshold: [" + softUpperThreshold.getUnit().toString() + "]: ");
            Double softUpperThresholdDouble = softUpperThreshold.doubleValue(softUpperThreshold.getUnit());
            if (!textSoftUpperThreshold.getText().equalsIgnoreCase(softUpperThresholdDouble.toString())) {
                textSoftUpperThreshold.setText(softUpperThresholdDouble.toString());
                softUpperThresholdSet = true;
            }
        ...
        hardLowerThreshold = (Measure<?, Quantity>) lowerThreshold.getThresholdLimit();
        hardUpperThreshold = (Measure<?, Quantity>) upperThreshold.getThresholdLimit();

        labelUpperThreshold.setText("Upper threshold: [" + hardUpperThreshold.getUnit().toString() + "]: ");
        labelUpperThreshold.setVisible(true);

        Double hardUpperThresholdDouble = hardUpperThreshold.doubleValue(hardUpperThreshold.getUnit());
        if (!textUpperThreshold.getText().equalsIgnoreCase(hardUpperThresholdDouble.toString())) {
            textUpperThreshold.setText(hardUpperThresholdDouble.toString());
            hardUpperThresholdSet = true;
        }

        textUpperThreshold.setVisible(true);
        labelLowerThreshold.setText("Lower threshold: [" + hardLowerThreshold.getUnit().toString() + "]: ");
        labelLowerThreshold.setVisible(true);

        Double hardLowerThresholdDouble = hardLowerThreshold.doubleValue(hardLowerThreshold.getUnit());
        if (!textLowerThreshold.getText().equalsIgnoreCase(hardLowerThresholdDouble.toString())) {
        ...
        if ((lowerThreshold instanceof LinearFuzzyThreshold) && upperThreshold instanceof LinearFuzzyThreshold) {
            if (hardUpperThresholdSet && hardLowerThresholdSet && softUpperThresholdSet && softLowerThresholdSet) {
                try {
        ...
        if ((lowerThreshold instanceof LinearFuzzyThreshold) && !(upperThreshold instanceof LinearFuzzyThreshold)) {
            softUpperThresholdSet = false;
            if (hardUpperThresholdSet && hardLowerThresholdSet && softLowerThresholdSet) {
                try {
                    this.checkInputForCorrectness();
                    this.setPageComplete(true);
                } catch (IncorrectThresholdInputException e) {
        ...
        if ((upperThreshold instanceof LinearFuzzyThreshold) && !(lowerThreshold instanceof LinearFuzzyThreshold)) {
            softLowerThresholdSet = false;
            if (hardUpperThresholdSet && hardLowerThresholdSet && softUpperThresholdSet) {
                try {
                    this.checkInputForCorrectness();
                    this.setPageComplete(true);
                } catch (IncorrectThresholdInputException e) {
    ...
    public void widgetSelected(SelectionEvent e) {
        ...
        scale.setToolTipText("" + scale.getSelection());
        ((SLOViewsWizard) this.getWizard()).setSelectedTimestepInterval(selectedTimestepInterval);
    }
